...
class runcrypt {
  ...
  u8_t state, thread_num, hash[20], iv[multicry_master::THREAD_MAX * 20];
  FILE *fp, *out;
  u8_t *key;
  hmac hmachandle;
  bool no_echo;
...
public:
  /*
      ...
      key:初始密钥序列
  */
  runcrypt(FILE *fp, FILE *out, u8_t *key, bool no_echo, u8_t thread_num=4)
      : fp(fp), out(out), key(key), state(0), hmachandle(0), no_echo(no_echo), thread_num(thread_num){};
