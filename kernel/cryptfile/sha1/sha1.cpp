...
void sha1hash::gethash(const u8_t *s) {
  ...
  for (u32_t i = 0; i < 80; ++i) {
    if (i < 20) {
      ...
      temp = lrot(h[nxt][0], 5) + f + 0x5A827999 + h[nxt][4] + w[i];
    } else if (i < 40) {
      f = h[nxt][1] ^ h[nxt][2] ^ h[nxt][3];
      temp = lrot(h[nxt][0], 5) + f + 0x6ED9EBA1 + h[nxt][4] + w[i];
    } else if (i < 60) {
      f = (h[nxt][1] & h[nxt][2]) | (h[nxt][1] & h[nxt][3]) |
          (h[nxt][2] & h[nxt][3]);
      temp = lrot(h[nxt][0], 5) + f + 0x8F1BBCDC + h[nxt][4] + w[i];
    } else {
      f = h[nxt][1] ^ h[nxt][2] ^ h[nxt][3];
      temp = lrot(h[nxt][0], 5) + f + 0xCA62C1D6 + h[nxt][4] + w[i];
    }
    h[nxt][4] = h[nxt][3];
    h[nxt][3] = h[nxt][2];
