...
Hashmaster::Hashmaster(HASH_TYPE type): type(type){
    switch (type){
    case SHA1: hasher=new sha1hash(); break;
    default: hasher=NULL; break;
    }
};
/*
getFileHash:返回文件哈希
...
hashres:结果哈希地址
*/
void Hashmaster::getStringHash(const u8_t * string, u32_t length, u8_t * hashres){
    hasher->reset();
    u32_t nnow = length;
    for (; nnow >= 64; nnow -= 64) {
