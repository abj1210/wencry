...
return:加密器地址组
*/
Aesmode **runcrypt::prepare_AES(u8_t *iv, size_t fsize, bool cmode)
{
  if (!cmode)
  ...
  for (int i = 0; i < threads_num; i++)
    mode[i] = aesfactory.createCryMaster(cmode, settings.get_ctype());
  return mode;
}
/*
release:释放空间
...
bool runcrypt::execute_encrypt(size_t fsize, u8_t *r_buf)
{
  ...
  Aesmode **mode = prepare_AES(iv, fsize, true);
  // 运行加密
...
bool runcrypt::execute_decrypt(size_t fsize)
{
  ...
  else
  {
    ...
    u8_t *iv = prepare_IV();
    Aesmode **mode = prepare_AES(iv, fsize, false);
    // 运行解密
