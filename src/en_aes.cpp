...
key:待加载的密钥
*/
aeshandle::aeshandle(keyhandle *key) : key(key) {}
encryaes::encryaes(keyhandle *key) : aeshandle(key) {}
/*
addroundkey:aes的密钥轮加操作
key:相应的轮密钥指针
*/
void aeshandle::addroundkey(const struct state &key) {
  w.datal ^= key.datal;
  w.datah ^= key.datah;
}
/*
...
w:待加解密数据块的地址
*/
void encryaes::encryaes_128bit(struct state &w) {
  this->w = w;
  for (int i = 0; i < 9; ++i)
    commonround(i);
  specround();
