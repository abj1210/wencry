...
针对多线程运行的特性,将单独的缓冲区打包成多线程缓冲区组(以下简称MBG),即代码中的`buffergroup`类.具体来说在MBG的构造函数中会预先加载各缓冲区的数据,每个缓冲区都对应一把锁,初始状态下除0号锁外其余锁全部处于锁定状态(即只有id为0的缓冲区可以操作文件).每个线程会根据独有的id访问相应缓冲区的数据.若数据处理完毕,则会调用MBG的`update_lst`尝试进行缓冲区更新.在函数`update_lst`中, 会首先尝试获取当前缓冲区的锁.若成功获取,则表明当前缓冲区可进行更新,即将旧数据插入输出文件并从输入文件获取新数据;否则线程将反复尝试直到获取.在更新完毕后将释放输出文件下一个需求的缓冲区--即序号为`(id+1)%size`的锁,使得下一缓冲区所属的线程能够进行更新.  
...
*v2.2 新增:部分函数改用类进行包装,重写多线程同步逻辑.(v2.2.1 重构部分代码)*  
