...
其中 -e 代表加密选项 fin 为输入文件的路径 code 为ase64编码后的16字节16进制数，若输入G则为程序生成 fout 为输出文件名，缺省为 fin 值。  
...
其中 -e 代表加密选项 fin 为输入文件的路径 code 为base64编码后的16字节16进制数 fout 为输出文件名，缺省为 [fin].wdec 。
